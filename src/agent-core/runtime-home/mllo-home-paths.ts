...
// 返回外部 agent/模型代理 trace 目录；观测数据不进入 session transcript 和模型上下文。
export function getMlloExternalTraceDir(options: MlloRuntimeHomeOptions = {}): string {
  return join(getMlloHomePath(options), "external-traces");
}
...
export function getMlloThreadRuntimeDir(args: MlloThreadRuntimePathArgs): string {
  return join(
    ...
    "threads",
    sanitizeThreadId(args.threadId),
  );
}
...
export function getMlloRuntimeHomeLayout(
  ...
  return {
    ...
    externalTraceDir: join(homePath, "external-traces"),
