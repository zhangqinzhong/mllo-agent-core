...
import type { MlloHookPhase } from "../hooks/agent-core-command-hook-protocol";
...
// 识别 ENOENT，用于把“还没有 session env”当成空环境处理。
...
// 按 phase 和 hook 顺序排序，保证每次 shell 注入环境的结果稳定。
...
// 读取当前 session 的环境脚本；空目录和空文件都不影响 shell 命令执行。
