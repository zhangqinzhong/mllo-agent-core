...
// 中文注释：workspace root 本身可能是 symlink，真实边界必须按 realpath 比较。
...
// 中文注释：工具会先检查再读写，缺失路径不能被当成系统错误提前打断。
...
// 中文注释：新文件还不存在时，必须用最近存在父目录判断 symlink 逃逸。
...
export function evaluateAgentCorePathPermission(
  ...
  if (!isPathInsideAgentWorkspace(context, resolved)) {
    return decision('ask', operation, 'Path is outside the active workspace roots.')
  }
...
// 中文注释：字符串路径检查挡不住 symlink，执行前还要按真实路径再验一次边界。
