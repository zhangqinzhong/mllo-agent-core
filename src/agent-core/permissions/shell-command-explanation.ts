...
function hasUnsafePipelineControl(command: string): boolean {
  return /(^|[^\\])(?:>>?|<<?|[;&])/.test(command) || /\|\|/.test(command);
}

function splitPipeline(command: string): string[] | null {
  if (hasUnsafePipelineControl(command)) {
    return null;
  }
  const parts = command
    .split(/(?<!\\)\|/)
...
function isReadonlyPipeline(command: string): boolean {
  ...
  // 这里只放行简单只读管道，目的是允许 `rg ... | head` 这类高效检索。
  return parts.every((part) => isReadonlyCommand(part));
}
  ...
  if (SHELL_CONTROL_RE.test(trimmed)) {
    if (isReadonlyPipeline(trimmed)) {
      return {
        ...
        reason: `Recognized read-only shell pipeline: ${preview}`,
      };
    }
    return {
      commandPreview: preview,
