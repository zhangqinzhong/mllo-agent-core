...
// 中文注释：stdio MCP server 是外部进程；这个类把 SDK 细节关在 core adapter 内部。
export class AgentCoreStdioMcpClient implements AgentCoreMcpClient {
  ...
  // 中文注释：只保存启动参数，不在构造阶段 spawn，避免仅构建 context 就启动外部进程。
  ...
  // 中文注释：列工具时才懒连接，让 MCP server 生命周期跟真实工具需求绑定。
  ...
  // 中文注释：工具调用沿用同一条连接，避免每次 tool call 都重新握手和丢 server 状态。
  ...
  // 中文注释：run controller 拥有自动创建的 client，结束时必须关闭子进程避免残留。
  ...
  // 中文注释：并发 list/call 共享同一个 connecting promise，避免同时 spawn 两个 server。
  ...
  // 中文注释：真正创建 SDK client/transport，并把 stderr 尾部保留下来方便连接失败诊断。
