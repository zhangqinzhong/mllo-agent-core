...
import { recordAgentCoreElicitationDecision } from './agent-core-elicitation-audit'
...
async function waitForElicitationDecision(
  ...
  try {
    return await Promise.race([args.onElicitationRequest(args.result), timeoutDecision])
  } finally {
...
export async function* resumeAgentCoreRunElicitation(
  ...
  const effectiveDecision =
    hookDecision.action === 'block'
      ...
      : decision
  await recordAgentCoreElicitationDecision({
