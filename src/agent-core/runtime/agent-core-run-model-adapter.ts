...
import { createAgentCoreLangfuseModelAdapter } from "../observability/agent-core-langfuse-model-adapter";
import type { AgentCoreLangfuseRunTrace } from "../observability/agent-core-langfuse-run-trace";
...
type AgentCoreRunModelAdapterOptions = {
  ...
  langfuseTrace?: AgentCoreLangfuseRunTrace;
};
...
export function createAgentCoreRunModelAdapter(
  ...
  return createAgentCoreLangfuseModelAdapter({
    model: createAgentCoreHttpModelAdapter(options.provider, fetchImpl),
    trace: options.langfuseTrace,
  });
}
