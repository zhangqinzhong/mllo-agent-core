...
function getCheckpointFileConflict(
  ...
  if (file.contentAfterWrite === undefined || currentContent === file.contentAfterWrite) {
    return {
      hasConflict: false
    }
  }
...
export async function previewAgentCoreCheckpointRecord(
  ...
  for (const file of record.files.slice().reverse()) {
    ...
    const needsCurrentContent = includeDiff || file.contentAfterWrite !== undefined
    const currentContent = needsCurrentContent
      ? await readCurrentContent(file.resolvedPath)
      : undefined
    const conflict = getCheckpointFileConflict(file, currentContent)
    const diffPreview =
      includeDiff && currentContent !== undefined
