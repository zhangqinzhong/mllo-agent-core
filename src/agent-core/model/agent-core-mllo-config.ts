...
} from '../../../shared/mllo-hook-protocol'
...
// 中文注释：摘要是设置页和 IPC 的稳定形状，hash 判断只在 main 进程做一次。
...
// 中文注释：保存 hook 时清理空 matcher 数组，避免配置文件积累无效字段。
...
// 中文注释：编辑定位必须唯一；同名 hook 会让 approve 无法判断用户批准的是哪一个。
