...
// 单个 worker 没声明 availability 时按可用处理，保持旧 worker 兼容。
...
// 汇总 worker 可用性，delegate/workflow 工具只要至少一个 worker 可用就保留。
...
// 把 worker 集合的 health check 映射成工具可用性，供 context builder 过滤 schema。
