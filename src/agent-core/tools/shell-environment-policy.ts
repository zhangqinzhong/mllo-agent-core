...
// 中文注释：校验结构化 env 名称，避免把非法键交给平台 shell 或 spawn。
function isValidShellEnvName(name: string): boolean {
  return VALID_ENV_NAME_RE.test(name)
}

// 中文注释：某些变量能注入 loader 或影响 Node/Electron 进程，结构化 env 不允许覆盖。
...
// 中文注释：远端 shell 默认不转发疑似 secret；需要每个 backend/host 显式放行。
...
// 中文注释：mllo 内部环境由运行态注入，模型不能通过 env 参数伪造 session 身份。
...
// 中文注释：构建 shell 进程环境；默认继承宿主环境，但对模型传入的 override 做白名单式校验。
